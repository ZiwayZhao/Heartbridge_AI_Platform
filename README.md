...
│   ├── ui/                 # shadcn/ui 基础组件库
│   ├── Layout.tsx          # 统一布局组件（包含侧边栏）
│   ├── AppSidebar.tsx      # 应用侧边栏导航（心桥 HeartBridge）
...
**数据表**: `invite_codes`
- `code`: 邀请码（唯一）
- `role`: 分配的角色（admin/therapist/parent）
...
**使用示例**:
1. 团队成员使用 `Upen666` 注册即可成为管理员
2. 注册时在"邀请码（可选）"字段输入
3. 系统自动验证并分配相应权限

---

### 2. RAG（检索增强生成）AI 聊天系统
...
**实现组件**: 
- `src/components/Layout.tsx` - 统一布局包装器
- `src/components/AppSidebar.tsx` - 侧边栏导航组件

**导航结构**:

**所有用户可见**:
- 🏠 首页 (`/`) - AI 聊天界面
- 👥 BCBA咨询师 (`/consultants`) - 查看咨询师列表

**管理员专用**:
- 📚 知识库管理 (`/knowledge`) - 上传、编辑、删除知识
- 👨‍⚕️ 咨询师管理 (`/bcba-management`) - 管理咨询师信息

**设计特点**:
- 侧边栏头部显示"心桥 HeartBridge"标志（蓝紫渐变色）
- 当前页面高亮显示（浅蓝色背景）
...
**权限控制**:
- 通过 `useAuth()` Hook 检查 `isAdmin` 状态
- 管理员菜单项仅在 `isAdmin = true` 时显示
- 路由级别的权限保护（AdminRoute 组件）
